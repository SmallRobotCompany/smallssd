...
from . import yolo_layers


DEFAULT_CFG_PATH = Path(__file__).parent / "default_cfg.cfg"
DEFAULT_CLASSES = 80
...
def generate_config(num_output_classes: int = 2) -> Iterable[str]:
    ...
    default_filters = (DEFAULT_CLASSES + 5) * 3
    ...
    return [
        l.replace(
            f"classes={DEFAULT_CLASSES}", f"classes={num_output_classes}"
        ).replace(f"filters={default_filters}", f"filters={num_filters}")
...
class YOLOConfiguration:
    ...
    def __init__(self, config_file: Iterable[str]) -> None:
...
def _create_route(config, num_inputs):
    ...
    source_layers = [
        layer if layer >= 0 else last + layer for layer in config["layers"]
    ]
...
def _create_yolo(config, num_inputs):
    ...
    anchor_dims = [
        (anchor_dims[i], anchor_dims[i + 1]) for i in range(0, len(anchor_dims), 2)
    ]
